...
public class CloudinaryFileUploadServiceImpl implements FileUploadService {
     ...
     * @param extraData additional data required to generate the file path (e.g., clientId, employeeId).
    ...
    public FileUploadResponse uploadFile(MultipartFile file, FileUploadCategory category, Map<String, String> extraData) {
        ...
        String filePath;
        switch (category) {
            case NOMINEE_PROFILE_IMAGE ->
                    filePath = String.format("image/nomineeId_%s/image", extraData.get("nomineeId"));
            case NOMINEE_DOCUMENT ->
                    filePath = String.format("document/nomineeId_%s/document", extraData.get("nomineeId"));
            default ->
